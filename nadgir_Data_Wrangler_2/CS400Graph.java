...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path source) {
            /*This copy constructor is made possible through setting the class
             *variable to the source's version of those respective variables.
             */
            ...
            for(int i = 0; i < dataSequence.size(); i++) {
                this.dataSequence.add(i, source.dataSequence.get(i));
            }
            this.end = source.start;
        }
        ...
        public void extend(Edge edge) {
            /*Current path becomes extended by making the distance equal to
             *the edge's weight, adding the edge's target data to the data
             *sequence, and then making the end the edge's target.
             */
    ...
    protected Path dijkstrasShortestPath(T start, T end) { 
        boolean startToEndPath = false;
        PriorityQueue<Path> pathQueue = new PriorityQueue(getVertexCount());
        Vertex startVertex = vertices.get(start);
        Path startingPath = new Path(startVertex);
        Hashtable <T, Vertex> visitedVertices =  new Hashtable();
        
        //Throws NoSuchElementExceptions if neither a start nor end element are found.
        if(containsVertex(start) == false) {
            throw new NoSuchElementException("No such start element.");
        }
        if (containsVertex(end) == false) {
            throw new NoSuchElementException("No such end element.");
        }
        ...
        while(pathQueue.isEmpty() == false) {
            /*
             * This if statement checks whether the element that is peeked 
             * in the Priority Queue is visited, and if not, it becomes a 
             * visited vertex.
             */
            if(!visitedVertices.contains(pathQueue.peek())) {
                visitedVertices.put(pathQueue.peek().start.data, pathQueue.peek().start);
                LinkedList<Edge> edges = pathQueue.peek().start.edgesLeaving;
                pathQueue.remove();
                while(edges.isEmpty() == false) {
                    Path edgeToTarget = new Path(edges.getFirst().target);
                    edgeToTarget.distance = edges.getFirst().weight;
                    pathQueue.add(edgeToTarget);
                    startingPath.extend(edges.getFirst());
                    if(startingPath.end.data == end) {
                        startToEndPath = true;
                        return startingPath;
                    }
                    else {
        ...
        if(startToEndPath == true) {
            return startingPath;
        }
        else {
            throw new NoSuchElementException("No path between start and end found.");
        }
    }
