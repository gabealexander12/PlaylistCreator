...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path source) {
      ...
      for (int i = 0; i < source.dataSequence.size(); ++i) {
        this.dataSequence.add(source.dataSequence.get(i));
      }
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    if (start == null || end == null || !containsVertex(start) || !containsVertex(end)) {
      throw new NoSuchElementException(); // if value of start or end does not exist in graph
    } // or is null NoSuchElementException is thrown
    Hashtable<T, Vertex> visitedNodes = new Hashtable<>(); // holds visited Nodes
    PriorityQueue<Path> pq = new PriorityQueue<>(); // priority queue to hold the paths
    Vertex c = vertices.get(start);
    Path pathStToEd = new Path(c);
    pq.add(pathStToEd);
    while (!pq.isEmpty()) {
      Path holdPath = new Path(pq.poll());
      c = holdPath.end;
      if (c.data.equals(end)) {
        pq.clear();
        return holdPath; // exits the while loop if the end is found
      }
      visitedNodes.put(c.data, c);
      for (int i = 0; i < c.edgesLeaving.size(); ++i) {
        Vertex s = c.edgesLeaving.get(i).target;
        if (!visitedNodes.containsKey(s.data)) { // visits all successing nodes that have not
          Path pathNew = new Path(holdPath); // been visited and adds them to the priority
          pathNew.extend(c.edgesLeaving.get(i)); // queue with a new extended path
          pq.add(pathNew);
        }
      }
    }
