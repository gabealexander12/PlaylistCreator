...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected Path dijkstrasShortestPath(T start, T end) {
		...
		Vertex sourceVertex = this.vertices.get(start);
		Vertex targetVertex = this.vertices.get(end);
		Path currentPath = new Path(sourceVertex);
		LinkedList<Vertex> check = new LinkedList<>();

		while (currentPath.end != targetVertex) {
			for (Edge e : currentPath.end.edgesLeaving) {
				if (check.contains(e.target) == false) {
			...
			while (check.contains(currentPath.end)) {
				currentPath = priorityQueue.poll();

			}
			check.add(currentPath.end);
