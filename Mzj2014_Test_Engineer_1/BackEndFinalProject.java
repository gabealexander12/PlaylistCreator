 ...
 * 
 * Specifically, this class contains a hash-table that holds Song objects-- a
 * music library-- and an instance of CS400Graph (from Project 3) to function as
 * a template for an "organized play-list".
 * 
 * This class contains methods to: populate the library from a CSV or from
 * scratch, set a "filter" to specify the type of music the user wants in their
 * play-list, and insert the filtered play-list into a graph with edges
 * connecting each song based on song similarity (determined by a simple
 * pseudo-algorithm). That graph is then spanned using Kruskal's MST algorithm
 * (the code for which is contained in Kruskal.java) thus resulting in a
 * "sorted" play-list with songs in an order based on similarity to one another.
 * 
...
// Back End fields
	...
	/**
	 * This sub-class represents the "filter" the user wishes to utilize to sort
	 * their music library. The class contains fields such as a start and end
	 * years-- this allows the user to say that they want music from a range of
	 * years. There is also an array of int: this allows the user to say they want
	 * music from just specific years. Also, there are genre and artist arrays to be
	 * used in the same way as the year selections.
	 *
	 */
	protected static class Filter {
		...
		// setters-- note there are not getters... the front end only needs to set the
		// filter
		...
		/**
		 * This method "clears" the filter class so as to be ready to be used again or
		 * called by the front end if the user wishes to restart.
		 */
	...
	/**
	 * This method is called by the front end to initialize the hash-table
	 * representing the song library. The front end can either call this method with
	 * a string representing a file path to a CSV, or the front end can pass 'null'
	 * to signify the user wishes to create a library from scratch. If an IO
	 * exception occurs then false is returned, else the library was initialized and
	 * true is returned.
	 * 
	 * @param filePath - a string representing what should be a valid file path
	 * @return true if library successfully initialized, false if not (i.e. invalid
	 *         file path passed)
	 */
	...
	/**
	 * This method returns the concatenation of the song artist and title to be used
	 * as the key for the hash-table... helpful for when making calls to songLibrary
	 * methods
	 * 
	...
	/**
	 * This method returns a string representing an organized play-list comprised of
	 * songs the user selected via the filters chosen.
	 * 
	...
	/**
	 * This method takes all of the contents of the song library and casts said
	 * collection to a stream. Then the stream is filtered based on parameters the
	 * user was previously prompted for. The user can select multiple parameters.
	 * Then the stream is cast to the static field filterSongs which will be used by
	 * graphFilteredSongs().
	 * 
	 */
	private static void filterSongs() {
		...
		songStream.filter(song -> Filter.selectYears.isEmpty() || Filter.selectYears.contains(song.getYear()))
	...
	/**
	 * This method adds every one of the filtered songs to the CS400Graph and then
	 * compares each song to every other song but itself. In comparing two given
	 * songs a "edge weight" is determined. The "edge" is then inserted into the
	 * graph between the two songs, for every possible song pairing.
	 */
	...
	/**
	 * This helper method compares two songs based on year released and genre and
	 * returns a cumulative difference value between the two songs that will be the
	 * weight in the CS400Graph object.
	 * 
	 ...
	 * @return - an int representing the cumulative "difference value" between the
	 *         two songs
	 */
	...
	/**
	 * This helper method is called from compareTwoSongs to implement the comparison
	 * of the genres of two songs. In order to compare the two genres, each genre is
	 * associated with a row and column in a 2D array-- accomplished by associating
	 * each genre with an index-- and the "difference value" between the two genres
	 * stored in each cell. "Difference values" range from 1 to 5.
	 * 
	 * Because genre A compared to B is the same as B to A, only half of the cells
	 * of the table need be filled. Should the comparison result in an "empty
	 * cell"-- i.e. the comparison is valid but fell in the empty half of the
	 * table-- the search is inverted guaranteeing a value found.
	 * 
	 * If one of the songs' rigid genres is uncategorized i.e. is Genre.NA, then the
	 * middle difference value of 3 is assigned to the songs. If the songs are of
	 * the same genre, then a value of 0 is assigned (there is no difference between
	 * the songs genre wise).
	 * 
	 * @param first  - an arbitrary name for one of the songs to be compared
	 * @param second - an arbitrary name for the other song to be compared
	...
	private static int compareTwoGenres(Song first, Song second) {
		...
		if (getSongRigidGenre(first).equals(Genre.NA) || getSongRigidGenre(second).equals(Genre.NA))
			return 3; // one of the songs does not contain an identifiable genre... cannot tell
						// difference, but there does exist a difference and 3 is the average
						// difference.
	...
	/**
	 * Simple helper method to assign rigid genre of song with corresponding index
	 * int
	 * 
	...
	/**
	 * This method is for exclusive use within the BackEnd class to get the rigid
	 * genre of a song, and if such has not yet been assigned, to assign such via a
	 * call to setRigidGenre()
	 * 
	 * @param song - the song object to get the rigid genre from, or assign to and
	 *             then retrieve
	...
	/**
	 * This method utilizes regexes to categorize the multitude of genres listed in
	 * a user supplied CSV to a handful of manageable genres that can be compared
	 * against one another.
	 * 
	 * @param song - the song object to assign a "rigid genre" to based upon its
	 *             more specific genre. If no match can be made using the regexes
	 *             the rigid genre is set to "NA" and will not be accounted for when
