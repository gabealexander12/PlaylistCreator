...
//TA: Brianna Cochran
//Lecturer: Gary Dahl
//Notes to Grader: This code had a bug that caused the path to be copied wrong 
//              while updating the path. By changing line 290 to create a new LinkedList vs 
//              a shallow copy it has fixed all issues.  
//				This code is identical to the 7:37pm submission aside from the one line change. 
...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected class Path implements Comparable<Path> {
		...
		public void extend(Edge edge) {
			this.distance = distance + edge.weight;
	...
	protected Path dijkstrasShortestPath(T start, T end) throws NoSuchElementException {
		int V = getVertexCount();// number of vertices in the graph
		LinkedList<Vertex> visited = new LinkedList<Vertex>();// linked list to hold vertex visited
		PriorityQueue<Path> pq = new PriorityQueue<Path>(V);// Priority Queue
		if (containsVertex(start) == false || containsVertex(end) == false) {
			throw new NoSuchElementException("That Start or End vertex is not stored in this Graph");
		}
		pq.add(new Path(vertices.get(start)));// adds start to pq
		Path shortest;// Path to return
		while (!pq.isEmpty()) {// algorithm to find shortest path
			shortest = pq.poll();// add peek to shortest path
			if (!visited.contains(shortest.end)) {// check to see if vertex has been visited before
				visited.add(shortest.end);// add to visited
				for (Edge edges : shortest.end.edgesLeaving) {// for loop to extend with new edges
					Path temp = new Path(shortest);// create new path
					temp.extend(edges);// extend the path
					pq.add(temp);// add path to pq
				}
			}
			if (shortest.end.data.equals(end)) {// check to see if end node has been reached
				return shortest;// return the shortest path
			}
		}
		throw new NoSuchElementException("There is no path connecting those two vertex.");
	}
