...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public void extend(Edge edge) {
      this.distance = this.distance + edge.weight; // update the distance
      this.dataSequence.add(edge.target.data); // update the dataSequence with target of edge
      this.end = edge.target; // update the end to be target of edge
    }
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    ...
    if (!this.containsVertex(start) || !this.containsVertex(end))
      throw new NoSuchElementException("Either start or end does not exist as a vertex");

    // initialize local variables
    Path shortestPath = new Path(this.vertices.get(start)); // initialize path with start vertex
    PriorityQueue<Path> frontier = new PriorityQueue<Path>();
    List<Vertex> alreadyVisited = new LinkedList<>();
    boolean endNotFound = true;

    // check if vertex to find shortest path to is itself
    if (this.vertices.get(start).data.equals(this.vertices.get(end).data))
      throw new NoSuchElementException("Start and end vertices are the same! No path exists!");

    // execute the algorithm
    while (endNotFound) {
      // iterate through the edges extending from the end of the current shortest path
      for (CS400Graph<T>.Edge i : shortestPath.end.edgesLeaving) {
        Path pathToExplore = new Path(shortestPath);
        pathToExplore.extend(i); // "add" the distance and target/end of the edge to the path
        if (!alreadyVisited.contains(pathToExplore.end))
          frontier.add(pathToExplore);
      }
      if (frontier.isEmpty()) // no unvisited vertices left, "end" was not found, throw exception
        throw new NoSuchElementException("No path between these vertices exisits!");
      // check if end vertex of path is target vertex, if yes, return path to end
      else if (frontier.peek().end.data.equals(end)) {
        shortestPath = frontier.peek();
        endNotFound = false;
        // unvisited nodes still exist, keep looking
      } else {
        shortestPath = frontier.peek(); // assign shortest path in queue to be shortestPath
        alreadyVisited.add(shortestPath.end); // shortestPath "end" is now "anchor" i.e. visited
        frontier.poll(); // path is now out-of-date and will be updated in next while-loop iteration
      }
    }
