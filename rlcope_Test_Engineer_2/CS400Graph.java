...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected class Path implements Comparable<Path> {
		...
		public Path(Path source) {
			...
			this.dataSequence = new LinkedList<>();
			for (T e : source.dataSequence) {
				this.dataSequence.add(e);
			}
			this.end = source.end;
		}
	...
	protected Path dijkstrasShortestPath(T start, T end) {
		PriorityQueue<Path> queue = new PriorityQueue<Path>(); // queue of Paths
		LinkedList<Vertex> visitedVertices = new LinkedList<Vertex>(); // list of visited vertices
		if (!this.containsVertex(start) || !this.containsVertex(end)) {
			throw new NoSuchElementException("No path to data exists.");
		}
		Vertex startingVertex = vertices.get(start); // starting vertex
		Path shortestPath = new Path(startingVertex); // original shortest path
		
		queue.add(shortestPath);
		while (!queue.isEmpty()) {
			shortestPath = queue.remove();
			if (!visitedVertices.contains(shortestPath.end)) {
				visitedVertices.add(shortestPath.end);
				for (Edge e : shortestPath.end.edgesLeaving) {
					Path copyShortestPath = new Path(shortestPath);
					copyShortestPath.extend(e);
					queue.add(copyShortestPath);
				}
			}
