...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    public boolean removeEdge(T source, T target) {
        ...
        if (sourceVertex == null || targetVertex == null)
            throw new IllegalArgumentException(
                "Cannot remove edge with vertices that do not exist");
        // find edge to remove
        Edge removeEdge = null;
        for (Edge e : sourceVertex.edgesLeaving)
            if (e.target == targetVertex)
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path source) {
            ...
            this.end = source.end;
//            this.start = vertices.get(source.dataSequence.get(0));
//            this.end = vertices.get(source.dataSequence.get(source.dataSequence.size() - 1));
//            this.distance = source.distance;
//            this.dataSequence = new LinkedList<>();
...
//            }
    ...
    protected Path dijkstrasShortestPath(T start, T end) {         
        if (!this.containsVertex(start) || !this.containsVertex(end)) {
            throw new NoSuchElementException("Path could not be found.");
        } 
        ...
        pathQueue.add(initialPath); // adds initial path to the path queue
        while (!(pathQueue.isEmpty())) {
            Path shortestPath = pathQueue.poll(); // takes shortest path from path queue
            if (visited.contains(shortestPath.end) == false) { // checks if this vertex has already been visited
                visited.add(shortestPath.end);
                int i = 0;
                while (i < shortestPath.end.edgesLeaving.size()) {
                    Path copy = new Path(shortestPath);
                    Edge endEdge = shortestPath.end.edgesLeaving.get(i);
                    copy.extend(endEdge);
                    pathQueue.add(copy);
                    i++;
                }
            }
            if (shortestPath.end.data.equals(end)) { // checks if the end node is found
                return shortestPath;
            }
        }
        throw new NoSuchElementException("Path could not be found");      
