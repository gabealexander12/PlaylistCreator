...
public class PlaylistWrangle<K, V> {
	...
	public static Hashtable<String, Song> readCSV(String filePath) throws IOException {
		...
		while (line != null) {
			...
			PlaylistWrangle.setSongYear(tokens[5]);
			PlaylistWrangle.setDuration(tokens[2]);
	 ...
	 * @return true if added
	 */
	public static boolean setDuration(String length) {
		if (!(length.equals("0")))
			return false;
		int inum = Integer.parseInt(length);
		song.setLength(inum);
		return true;
	}
	 ...
	 * @return true if added
	 */
	public static boolean setSongYear(String year) {
		if (!(year.equals("0")))
			return false;
	...
	public static void main(String[] args) {
		Hashtable<String, Song> table = new Hashtable<String, Song>();
		try {
			table = readCSV("src/music.csv");
		} catch (IOException e) {
			e.printStackTrace();
		}
		PlaylistWrangle.setArtist("Green Day");
		PlaylistWrangle.setGenre("Punk Rock");
		PlaylistWrangle.setDuration("212");
		...
		System.out.println(hashTable.get("Green DayWhatsername").getLength());// should print 212
		System.out.println("****************************************");
		System.out.println("Testing readCSV input: ");
		System.out.println("");
		System.out.println(table.get("50 Cent / Mobb DeepOutta Control").getArtist());// should print 50cent
		System.out.println(table.get("50 Cent / Mobb DeepOutta Control").getGenre());// should print rap
		System.out.println(table.get("50 Cent / Mobb DeepOutta Control").getAlbum());// should print The Massacre (Ecopac Reissue Explicit)
		System.out.println(table.get("50 Cent / Mobb DeepOutta Control").getSong());// should print Outta Control
		System.out.println(table.get("50 Cent / Mobb DeepOutta Control").getYear());// should print 2005
		System.out.println(table.get("50 Cent / Mobb DeepOutta Control").getLength());// should print 247.77098
		System.out.println("****************************************");
	}
}
